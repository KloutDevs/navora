...
// HUD Modal component for dangerous action confirmation
export function HUDModal({
  confirmation,
  onApprove,
  onDeny
}: {
  confirmation: { id: string; action: string; details: Record<string, unknown>; domain: string };
  onApprove: () => void;
  onDeny: () => void;
}) {
  return (
    <div className="hud-modal">
      <div className="hud-dialog">
        ...
        <p>
          The AI wants to perform <strong>{confirmation.action.replace('_', ' ')}</strong> on{' '}
          <strong>{confirmation.domain}</strong>.
        </p>
        <div className="hud-details">
          {Object.entries(confirmation.details).map(([key, value]) => (
            <div key={key}>
              <span className="key">{key}:</span> <span className="value">{String(value)}</span>
            </div>
          ))}
        </div>
